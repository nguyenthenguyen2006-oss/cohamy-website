...
const mutable=new Set(['memberships','partner_assignments','partner_tags','partner_contacts','dealer_addresses','dealer_carts','support_tickets','website_orders','opportunities','private_documents','tasks','care_schedules','partner_library','partner_applications','partner_invitations']);
...
export async function mergeConfirm(user:Principal,input:unknown) {
  ...
  return(await database()).transaction(async sql=>{
    ...
    for(const table of ['memberships','dealer_carts','support_tickets','website_orders','opportunities','partner_applications'])await sql.query(`UPDATE cohamy_crm.${table} SET organization_id=$2,version=version+1 WHERE organization_id=ANY($1::uuid[])`,[sourceIds,target]);
