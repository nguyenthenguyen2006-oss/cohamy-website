...
try{
 ...
 await check('New database has no demo requests or tasks',async()=>{const r=await request('/api/crm/work/orders');assert.equal(r.status,200);assert.equal((await r.json()).total,0);const tasks=await request('/api/crm/work/tasks');assert.equal(tasks.status,200);assert.equal((await tasks.json()).items.length,0);});
