...
export async function CrmDashboard({user}:{user:Principal}){
 ...
 const roleQueue=internal?await roleDashboard(user):[];
 ...
 {roleQueue.length>0&&<section className="work-section"><div className="work-section-title"><div><h2>Hàng đợi theo vai trò</h2><p>Mỗi số mở đúng nguồn chứng từ trong phạm vi hiện tại.</p></div>{can(user,'reports.read')&&<Link href="/crm/reports">Mở báo cáo vận hành →</Link>}</div><ul className="work-task-list">{roleQueue.map(item=><li key={item.label}><Link href={item.href}>{item.label}</Link><strong>{item.value}</strong></li>)}</ul></section>}
